/*NECESITO HACER QUE EL PROGRAMA INICIE AL ENCENDER LA INTEL EDISON
  Y QUE SE RECONOZCA CADA GESTO Y UN CODIGO PARA CADA GESTONE EL .ON*/
...
myo.quickConnect(function (err, id) {
    ...
    myo.event.on('classifier', function (data) { // aca debe ir lo que quiero que se ejecute cada vez
        ...
        console.log('classifier', 'working',data);   // que un emitter accione un evento
    });

    interaction(id); // en interaction deben ir los emitters
    /*
...
function interaction(id) { // interaction es un bucle por lo que aqui debo poner los emitters
   ...
   */
   ...
   myo.connected[id].info(function (err, data) {
    ...
    myo.connected[id].classifier(function () {
