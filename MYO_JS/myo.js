...
function startScan(callback) {
    if (ble) {
        ...
        setTimeout(function () { // esperando a que se conecte
            startScan(callback);
        }, 100);
    }
}
...
function registerControl(Control, peripheral, callbackMain) {
    Control.discoverCharacteristics([services.MyoInfoCharacteristic, services.FirmwareVersionCharacteristic, services.CommandCharacteristic], function (error, characteristics) {
        if (characteristics.length == 3) {
            var myoInfo = characteristics[0]; // el orden si esta bien? lo cambie
    ...
    Emg.discoverCharacteristics([services.EmgData0Characteristic, services.EmgData1Characteristic, services.EmgData2Characteristic, services.EmgData3Characteristic], function (error, characteristics) {
        ...
        for (var i = 0; i < 4; i++) {
            EmgChar[i].on('read', function (data) {
                ...
                eventEmitter.emit('emg' + i, { id: peripheral.id, objEMG: objEMG });
            });
        }
...
noble.on('discover', function (peripheral) {
    ...
    peripheral.on('connect', function () {
        ...
        peripheral.discoverServices([bluetoothStandard.DeviceService, bluetoothStandard.BatteryService, services.ControlService, services.ImuDataService, services.ClassifierService, services.EmgDataService], function (error, services) {
  // cambie el orden
